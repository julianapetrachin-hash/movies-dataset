...
# --- CSS DEFINITIVO (Ajuste de Topo e Separação de Gráficos) ---
st.markdown("""
    <style>
    /* 1. Remove o espaço padrão do Streamlit no topo */
    ...
    /* 4. Containers de Gráficos (Efeito Flutuante) */
    .plot-container {
...
try:
    ...
    with m5: st.markdown(f'<div class="card-kpi"><div class="label-kpi">Pendentes</div><div class="value-kpi" style="color:#ff4b4b">{pendentes}</div><div class="sub-kpi">Aguardando Fechamento</div></div>', unsafe_allow_html=True)
    ...
    with g2:
        st.subheader("🏢 Status de Saúde (Separado por CD)")
        st.markdown('<div class="plot-container">', unsafe_allow_html=True)
        df_tree = df_filt[df_filt['v_1c'] != 0].copy()
        df_tree['cd_label'] = df_tree['cd'].astype(str).str.replace(r'\.0$', '', regex=True)
        
        # HIERARQUIA: Aqui garantimos que os CDs apareçam separados dentro dos blocos de Tipo
        fig_t = px.treemap(df_tree, 
                           path=['tipo_clean', 'cd_label'], # SEPARA TIPO E DEPOIS OS CDs

...
st.markdown("""
    ...
    .target-line { position: absolute; height: 100%; width: 2px; background: #00f2ff; z-index: 3; box-shadow: 0 0 8px #00f2ff; }
    .target-label { font-size: 10px; color: #8b949e; }
    </style>
...
try:
    ...
    # 2. CARDS SUPERIORES (FINANCEIRO)
    p1c = df_filt['v_1c'].sum(); vfal = df_filt['v_falta'].sum(); fat = df_filt['v_fat'].sum()
    perda_total = p1c + vfal
    perc_falta = (vfal / perda_total * 100) if perda_total != 0 else 0
    media_un = perda_total / len(df_filt) if len(df_filt) > 0 else 0
    ...
    with k3: st.markdown(f'<div class="card-kpi"><div class="label-kpi">Média por Unidade</div><div class="value-kpi">R$ {media_un:,.0f}</div><div class="sub-kpi">Base: {len(df_filt)} Unidades</div></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # 3. CARDS DE UNIDADES (TOTAL, FINALIZADAS, PENDENTES)
    u1, u2, u3 = st.columns(3)
    total_uds = len(df_filt); fechadas = df_filt['is_fin'].sum(); pendentes = total_uds - fechadas
    target_pos = 70 # Alvo em 70%

    with u1: st.markdown(f'<div class="card-kpi"><div class="label-kpi">Total Unidades</div><div class="value-kpi">{total_uds}</div><div class="sub-kpi">Cadastradas no Filtro</div></div>', unsafe_allow_html=True)
    with u2: 
    ...
    # 4. GRÁFICOS DO MEIO (BARRAS + TREEMAP)
    st.markdown("<br>", unsafe_allow_html=True)
    g1, g2 = st.columns([1, 1.1])
    
    with g1:
        st.subheader("📊 Resultado Consolidado por Processo")
        df_proc = df_filt.groupby('tipo_clean')[['v_1c', 'v_falta']].sum().sum(axis=1).reset_index(name='res')
        fig_b = px.bar(df_proc, x='tipo_clean', y=df_proc['res'].abs(), text='res', color='tipo_clean',
                       color_discrete_map={'CD':'#3a7bd5','LV':'#7000ff','DQS':'#00f2ff'})
        fig_b.update_traces(texttemplate='R$ %{text:,.0f}', textposition='outside')
        fig_b.update_layout(template="plotly_dark", height=400, showlegend=False, yaxis_visible=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig_b, use_container_width=True)

    with g2:
        st.subheader("🏢 Status de Saúde (Tipo > CD)")
        df_tree = df_filt.copy()
        df_tree['res'] = df_tree['v_1c'] + df_tree['v_falta']
        df_tree = df_tree[df_tree['res'] != 0].copy()
        df_tree['cd_lbl'] = df_tree['cd'].astype(str).str.replace(r'\.0$', '', regex=True)
        fig_t = px.treemap(df_tree, path=['tipo_clean', 'cd_lbl'], values=df_tree['res'].abs(), color='tipo_clean',
        ...
        fig_t.update_layout(template="plotly_dark", height=400, margin=dict(t=20, b=10, l=0, r=0))
    ...
    with b1:
        ...
        st.dataframe(df_ex.style.apply(lambda r: ['background-color: #451a1a' if r['v_1c'] < 0 else 'background-color: #1a4523']*len(r), axis=1),
                     column_config={"v_1c": st.column_config.NumberColumn("Resultado", format="R$ %.2f"),
                                    "%": st.column_config.NumberColumn("%", format="%.4f%%"),
                                    "v_falta": st.column_config.NumberColumn("Falta", format="%.0f")},
    ...
    with b2:
        ...
        fig_p.update_layout(template="plotly_dark", height=500, margin=dict(t=50, b=50, l=0, r=0), showlegend=True,
